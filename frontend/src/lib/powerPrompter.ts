...
export const DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE: PowerPrompterDetailerStage[] =
  DEFAULT_POWER_PROMPTER_GENERATION_CONTROLS.detailerPipeline!.map((stage) => ({ ...stage }));

function createDetailerStageId(): string {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return `detail-${crypto.randomUUID()}`;
  }
  return `detail-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}

export function createPowerPrompterDetailerStage(
  template?: Partial<PowerPrompterDetailerStage>,
): PowerPrompterDetailerStage {
  const base = DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE[1] || DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE[0];
  return {
    ...base,
    ...template,
    id: createDetailerStageId(),
  };
}
...
export const POWER_PROMPTER_MAX_COMPLETION_SOUND_VOLUME = 1;
...
function normalizeVideoDimension(value: unknown, fallback: number, multiple: number): number {
  const clamped = clampInteger(value, fallback, 64, 8192);
  return Math.max(multiple, Math.round(clamped / multiple) * multiple);
}

function normalizeVideoFrames(value: unknown, fallback: number, stride: number): number {
  const clamped = clampInteger(value, fallback, 1, 16385);
...
function normalizePowerPrompterVideoControls(rawVideo: unknown): PowerPrompterVideoControls {
  ...
  const rtxQuality: PowerPrompterVideoControls['postprocess']['rtxQuality'] = rtxQualityRaw === 'LOW'
  ...
  const sourceWidth = clampInteger(video.sourceWidth, 0, 0, 32768);
  ...
  return {
    ...
    denoise: clampNumber(video.denoise, defaults.denoise, 0.01, 1),
    ...
    decodeMode,
    decodeTileSize: normalizeVideoDimension(video.decodeTileSize, defaults.decodeTileSize, 32),
    decodeOverlap: clampInteger(video.decodeOverlap, defaults.decodeOverlap, 0, 4096),
    temporalTileSize: clampInteger(video.temporalTileSize, defaults.temporalTileSize, 8, 4096),
    temporalOverlap: clampInteger(video.temporalOverlap, defaults.temporalOverlap, 4, 4096),
    postprocess: {
      ...
      interpolationMultiplier: clampInteger(postprocess.interpolationMultiplier, defaults.postprocess.interpolationMultiplier, 2, 16),
      upscaleMode,
    ...
    wan: {
      ...
      highLoraStrength: clampNumber(wan.highLoraStrength, defaults.wan.highLoraStrength, -100, 100),
      lowLoraStrength: clampNumber(wan.lowLoraStrength, defaults.wan.lowLoraStrength, -100, 100),
      ...
      steps,
      splitStep: clampInteger(wan.splitStep, defaults.wan.splitStep, 1, Math.max(1, steps - 1)),
      cfg: clampNumber(wan.cfg, defaults.wan.cfg, 0, 100),
      shift: clampNumber(wan.shift, defaults.wan.shift, 0, 100),
    ...
    ltx: {
      ...
      distilledLoraStrength: clampNumber(ltx.distilledLoraStrength, defaults.ltx.distilledLoraStrength, -100, 100),
      promptLora: String(ltx.promptLora || '').trim().replace(/\\/g, '/'),
      promptLoraStrength: clampNumber(ltx.promptLoraStrength, defaults.ltx.promptLoraStrength, -100, 100),
      latentUpscaleModel: String(ltx.latentUpscaleModel || '').trim().replace(/\\/g, '/'),
      audioVae: String(ltx.audioVae || '').trim().replace(/\\/g, '/'),
      baseCfg: clampNumber(ltx.baseCfg, defaults.ltx.baseCfg, 0, 100),
      refineCfg: clampNumber(ltx.refineCfg, defaults.ltx.refineCfg, 0, 100),
...
function normalizePowerPrompterHiresFixControls(rawHiresFix: unknown): PowerPrompterHiresFixControls {
  ...
  return {
    ...
    upscaler: String(hiresFix.upscaler || defaults.upscaler).trim() || defaults.upscaler,
    resizeMode: String(hiresFix.resizeMode || '').trim().toLowerCase() === 'dimensions' ? 'dimensions' : 'scale',
    scaleBy: clampNumber(hiresFix.scaleBy, defaults.scaleBy, 1, 8),
    targetWidth: normalizeHiresDimension(hiresFix.targetWidth),
    targetHeight: normalizeHiresDimension(hiresFix.targetHeight),
    steps: clampInteger(hiresFix.steps, defaults.steps, 0, 10000),
    denoise: clampNumber(hiresFix.denoise, defaults.denoise, 0, 1),
    cfg: clampNumber(hiresFix.cfg, defaults.cfg, 0, 100),
    samplerName: String(hiresFix.samplerName || defaults.samplerName).trim() || defaults.samplerName,
    scheduler: String(hiresFix.scheduler || defaults.scheduler).trim() || defaults.scheduler,
  };
}

function normalizeDetailerDimension(value: unknown, fallback: number): number {
  const clamped = clampInteger(value, fallback, 64, 16384);
...
function normalizePowerPrompterDetailerPipeline(
  ...
  const source = Array.isArray(rawPipeline)
    ? rawPipeline
    : DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE.map((stage) => ({
      ...stage,
  ...
  return source.flatMap((entry, index) => {
    ...
    const preset = DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE.find((candidate) => (
    ...
    )) || DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE[1] || DEFAULT_POWER_PROMPTER_DETAILER_PIPELINE[0];
    ...
    return [{
      id,
      enabled: isLegacyDefaultEyeStage ? false : normalizeDetailerBoolean(stage.enabled, preset.enabled),
...
function normalizePowerPrompterOutputUpscaleControls(rawOutputUpscale: unknown): PowerPrompterOutputUpscaleControls {
  ...
  return {
    ...
    modelName: String(outputUpscale.modelName || defaults.modelName).trim().replace(/\\/g, '/') || defaults.modelName,
    maxDimension: clampInteger(outputUpscale.maxDimension, defaults.maxDimension, 512, 16384),
  };
}
...
export function normalizePowerPrompterGenerationControls(rawControls: unknown): PowerPrompterGenerationControls {
  ...
  return {
    ...
    workflowResources: normalizePowerPrompterWorkflowResources((controls as any).workflowResources),
