...
import {
  ...
  hasAcknowledgedPendingBatchControl,
  hasAcknowledgedUnsubmittedQueueControl,
  hasOnlyLocalStagedQueue,
  hasQueueSnapshotForCurrentSocket,
  isLocalOnlyPausedQueue,
  planLocalQueueStage,
  resolveActiveQueueControlRequestId,
  resolveQueueStartStopDisposition,
...
export function queuePausedAfterRemovingWork(
  wasPaused: boolean,
  items: ReadonlyArray<Pick<QueueStackItem, 'requestId' | 'promptIndex' | 'status' | 'exiting'>>,
  removedRequestIds: ReadonlySet<string>,
  removedPromptIndicesByRequest?: ReadonlyMap<string, ReadonlySet<number>>,
): boolean {
  if (!wasPaused) return false;
  return items.some((item) => {
    if (item.exiting || (item.status !== 'pending' && item.status !== 'running')) return false;
    const requestId = String(item.requestId || '').trim();
    if (removedRequestIds.has(requestId)) return false;
    const removedIndices = removedPromptIndicesByRequest?.get(requestId);
    return !removedIndices?.has(Math.max(0, Math.floor(Number(item.promptIndex) || 0)));
  });
}

export function queuePausedFromBackendSnapshot(
  snapshotPaused: boolean,
  hasLiveBackendPrompts: boolean,
  hasLocalStagedWork = false,
): boolean {
  return (snapshotPaused && hasLiveBackendPrompts)
    || (!hasLiveBackendPrompts && hasLocalStagedWork);
}

export function mergeBackendQueueSnapshotWithLocalStage(
  backendItems: ReadonlyArray<QueueStackItem>,
  previousItems: ReadonlyArray<QueueStackItem>,
  backendRequestIds: ReadonlySet<string>,
  localStagedRequestIds: ReadonlySet<string>,
): QueueStackItem[] {
  return [
    ...backendItems,
...
export function queueVisualForSnapshotWithLocalStage(
  backendVisual: QueueVisualState | null,
  previousVisual: QueueVisualState | null,
  stagedItems: ReadonlyArray<QueueStackItem>,
  requestMeta: ReadonlyMap<string, QueueRequestMeta>,
  localStagedRequestIds: ReadonlySet<string>,
): QueueVisualState | null {
  if (backendVisual) return backendVisual;
  const previousRequestId = String(previousVisual?.requestId || '').trim();
  if (previousVisual && stagedItems.some((item) => String(item.requestId || '').trim() === previousRequestId)) {
    return previousVisual;
  }
  const firstStagedItem = stagedItems.find((item) => localStagedRequestIds.has(String(item.requestId || '').trim()));
  if (!firstStagedItem) return null;
  const requestId = String(firstStagedItem.requestId || '').trim();
  ...
  return {
    ...
    promptIds: meta.prompts.map(() => ''),
    promptSeeds: meta.prompts.map(() => 0),
    activeIndex: 0,
    jobProgress: 0,
    updatedAt: Date.now(),
  };
...
export const PowerPrompter = ({ overlayMode = false, isActive = true, queueManagerActive, onOpenQueueManager, onOpenPrompter }: PowerPrompterProps) => {
  ...
  const pendingBatchAdmissionsRef = useRef(0);
  const activeQueueBatchRequestIdRef = useRef('');
  const activeQueueBatchGroupRequestIdsRef = useRef<string[]>([]);
  const queueStartSequenceRef = useRef(0);
  const queueStartStopRequestedRef = useRef(0);
  const queueStartStopConfirmedRef = useRef(0);
  const queueStartStopFailedRef = useRef(0);
  const queueStopInFlightRef = useRef(false);
  ...
  const pendingBackendQueueControlsRef = useRef(new Map<string, {
    resolve: (value: any) => void;
    reject: (reason?: unknown) => void;
    timer: ReturnType<typeof setTimeout>;
  ...
  const backendQueueSnapshotPausedRef = useRef(false);
  const backendQueueActivePromptRequestIdsRef = useRef<Set<string>>(new Set());
  ...
  const persistSettings = async (
    ...
    options?: { silent?: boolean; broadcast?: boolean; baseSettings?: PowerPrompterSettings; changes?: Record<string, unknown> }
  ): Promise<boolean> => {
    const changes = options?.changes ?? diffPowerPrompterSettings(options?.baseSettings ?? settings, nextSettings);
    ...
    const write = async (): Promise<boolean> => {
      try {
        ...
        const payload = await response.json() as { settings?: PowerPrompterSettings };
        if (!payload?.settings) throw new Error('The settings response was incomplete');
        if (writeSeq === settingsWriteSeqRef.current) {
          const committed = normalizePowerPrompterSettings(payload.settings);
          setSettings(committed);
          if (options?.broadcast !== false) broadcastSettingsSync(committed);
        }
        return true;
      } catch {
        if (!options?.silent) showToast('Failed to save settings', 'error');
        return false;
      }
    };
  ...
  const rejectAllPendingQueueRequests = (reason: string) => {
    ...
    pendingBackendQueueControlsRef.current.clear();
  ...
  const clearQueueManagerLiveDisplay = (reason: string) => {
    ...
    backendQueueSnapshotPausedRef.current = false;
    backendQueueActivePromptRequestIdsRef.current = new Set();
    backendQueueSnapshotActiveUntilRef.current = 0;
    backendQueueSnapshotSignatureRef.current = '';
    backendQueuePauseRequestedRef.current = false;
  ...
  const cancelTrackedQueueRequest = (
    ...
    scheduleRecoverableQueueSnapshotPersist({ paused: nextPaused, clearWhenEmpty: true, delayMs: 50 });
  ...
  const applyBackendQueueSnapshot = (snapshotInput: any) => {
    ...
    const activePromptRequestIds = new Set<string>();
    ...
    for (const rawRequest of powerPrompterRequests) {
      ...
      for (const entry of rawPrompts) {
        ...
        activePromptRequestIds.add(requestId);
    ...
    backendQueueActivePromptRequestIdsRef.current = activePromptRequestIds;
    const hasLiveBackendSnapshotWork = activeBackendIds.size > 0 || backendStackItems.length > 0;
    backendQueueSnapshotPausedRef.current = snapshot?.paused === true && hasLiveBackendSnapshotWork;
    backendQueueSnapshotActiveUntilRef.current = hasLiveBackendSnapshotWork
      ? Date.now() + 60000
      : 0;
    backendQueuePauseRequestedRef.current = backendQueueSnapshotPausedRef.current;
    const backendRequestOrder: string[] = powerPrompterRequests
      .map((request: any) => normalizeRequestId(request?.requestId))
      .filter(Boolean);
    ...
    const backendQueueStackItems = applyQueueStackRunningState(
    ...
    const stagedItemsAtReceipt = mergeBackendQueueSnapshotWithLocalStage(
      [], queueStackItemsRef.current, backendRequestIds, localStagedRequestIds,
    );
    const hasLocalStagedWork = stagedItemsAtReceipt.length > 0;
    const nextQueuePaused = queuePausedFromBackendSnapshot(
    ...
    logQueueDebug('ws:queue_snapshot:applied', {
      ...
      localStagedItems: stagedItemsAtReceipt.length,
  ...
  const hasCurrentBackendQueueSnapshot = (): boolean => {
    const ws = prompterWsRef.current;
    return hasQueueSnapshotForCurrentSocket(
      prompterWsReadyRef.current, ws, initialQueueSnapshotWsRef.current,
      pendingBatchAdmissionsRef.current,
    );
  };
  ...
  const requestQueueThroughWebSocket = async (
    ...
    const editorSnapshot = normalizeQueueEditorSnapshot(stateOverride?.editorSnapshot)
      || queueRequestMetaRef.current.get(requestId)?.editorSnapshot;
    const source = resolveQueueDispatchSource(editorSnapshot, cardDocumentRef.current, currentFileRef.current);
    const sourceDocument = source.document;
    const sourceFile = typeof stateOverride?.sourceFile === 'string' ? stateOverride.sourceFile : source.file;
    const fallbackSetId = clampQueueSetId(
      stateOverride?.activeQueueSet
      ?? (stateOverride as any)?.activeSetId
      ?? sourceDocument.activeQueueSet
    );
    const normalizedGeneration = normalizePowerPrompterGenerationControls(
      (stateOverride as any)?.generation ?? sourceDocument.generation
    );
    const pipeline = normalizeUmbraUiPipelineSelection(
      (stateOverride as any)?.pipeline ?? sourceDocument.pipeline,
      {
        feature: 'txt2img',
        modelFamily: (stateOverride as any)?.modelFamily ?? sourceDocument.modelType,
    ...
    const normalizedStyleSeedMode = String(
      ...
      ?? (sourceDocument as any).styleSeedMode
      ?? 'same'
    ...
    const state = {
      ...
      editorSnapshot: editorSnapshot
        || createQueueEditorSnapshot(sourceDocument, source.file, {
  ...
  const buildQueueWebSocketStateFromMeta = (meta: QueueRequestMeta) => {
    ...
    const sourceDocument = source.document;
    const groupGeneration = normalizePowerPrompterGenerationControls(meta.generationByPrompt[0] ?? sourceDocument.generation);
    ...
    return {
      ...
      styleSeedMode: String((sourceDocument as any).styleSeedMode || 'same') === 'different' ? 'different' : 'same',
      pipeline,
      modelFamily: pipeline.modelFamily,
  ...
  const requestQueueBatchThroughWebSocket = async (requestIdsInput: unknown[]): Promise<any> => {
    ...
    activeQueueBatchRequestIdRef.current = batchRequestId;
    activeQueueBatchGroupRequestIdsRef.current = requestIds;
  ...
  const saveSettings = async (newSettings: PowerPrompterSettings, baseSettings: PowerPrompterSettings) => {
    ...
    await persistSettings(merged, { changes: diffPowerPrompterSettings(baseSettings, newSettings) });
  };
      ...
      try {
        ...
        if (
          ...
          const changes = Object.fromEntries(
            ['editorMode', 'queueTraversalMode', 'queueDiversity', 'queuePromptLimit',
              'generationCompleteSoundEnabled', 'generationCompleteSoundStyle']
              .filter((key) => JSON.stringify(normalized[key as keyof PowerPrompterSettings]) !== JSON.stringify(rawSettings?.[key]))
              .map((key) => [key, normalized[key as keyof PowerPrompterSettings]]),
          );
          void persistSettings(normalized, { silent: true, broadcast: false, changes });
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      initialQueueSnapshotWsRef.current = null;
      ...
      ws.onmessage = (event) => {
        ...
        if (messageType === 'queue_snapshot') {
          const queueSnapshot = payload.snapshot && typeof payload.snapshot === 'object'
            ? payload.snapshot : payload;
          if (!Array.isArray(queueSnapshot.requests)) return;
          applyBackendQueueSnapshot(payload);
          initialQueueSnapshotWsRef.current = ws;
          const pendingAdmissions = Number(queueSnapshot.pendingBatchAdmissions);
          pendingBatchAdmissionsRef.current = Number.isFinite(pendingAdmissions)
            ? Math.max(0, Math.floor(pendingAdmissions)) : 0;
          return;
        }
        ...
        if (messageType === 'queue_batch_forwarded') {
          ...
          pendingQueueRequestsRef.current.delete(requestId);
          if (payload.success === false) {
            if (payload.canceled === true && requestId === activeQueueBatchRequestIdRef.current
              && queueStopInFlightRef.current
              && queueStartStopRequestedRef.current === queueStartSequenceRef.current) {
        ...
        if (messageType === 'queue_cancel_result') {
          ...
          if (pendingBackendControl) {
            ...
            if (payload.success === false || payload.backendHandled !== true) {
              pendingBackendControl.reject(new Error(String(payload.error || 'Backend queue cancel failed.')));
            } else {
          ...
          if (queueStopInFlightRef.current && activeQueueBatchRequestIdRef.current
            && Array.isArray(payload.canceledBatchRequestIds)
            && payload.canceledBatchRequestIds.includes(activeQueueBatchRequestIdRef.current)) {
            queueStartStopConfirmedRef.current = queueStartSequenceRef.current;
          }
          ...
          const nextPaused = queuePausedAfterRemovingWork(
            queuePausedRef.current,
            queueStackItemsRef.current,
            new Set(effectiveCanceledRequestIds),
          );
          pendingQueueCancelScopeRef.current = [];
      ...
      ws.onclose = (event) => {
        ...
        if (initialQueueSnapshotWsRef.current === ws) {
          initialQueueSnapshotWsRef.current = null;
          pendingBatchAdmissionsRef.current = 0;
        }
        if (prompterWsRef.current !== ws) return;
  ...
  const requestBackendQueueControlThroughWebSocket = (
    ...
    requestIds: string[],
    targetBridgeId?: string,
    cancelPendingBatchRequestId?: string,
  ): Promise<any> => {
    if (!prompterWsReadyRef.current || !prompterWsRef.current || prompterWsRef.current.readyState !== WebSocket.OPEN) {
    ...
    const requestId = createRequestId();
    const target = resolveQueueControlTarget(targetBridgeId, 'pipeline');
    return new Promise<any>((resolve, reject) => {
      const timer = setTimeout(() => {
        pendingBackendQueueControlsRef.current.delete(requestId);
        reject(new Error('Timed out waiting for backend queue control. Check the queue state before retrying.'));
      }, 20_000);
      pendingBackendQueueControlsRef.current.set(requestId, { resolve, reject, timer });
      if (action === 'cancel') pendingQueueCancelScopeRef.current = normalizedRequestIds;
      else pendingQueueClearFutureScopeRef.current = normalizedRequestIds;
      const sent = sendPrompterWsMessage({
      ...
      if (!sent) {
        clearTimeout(timer);
        pendingBackendQueueControlsRef.current.delete(requestId);
        if (action === 'cancel') pendingQueueCancelScopeRef.current = [];
        else pendingQueueClearFutureScopeRef.current = [];
        reject(new Error('Power Prompter queue tracker disconnected before the control was sent.'));
      }
    });
  };
  ...
  const handleStartQueuedDispatch = async () => {
    ...
    if (!hasCurrentBackendQueueSnapshot()) {
      showToast('Waiting for backend queue state. Try again shortly.', 'error');
      return;
    }
    ...
    const startSequence = ++queueStartSequenceRef.current;
    ...
    try {
      ...
      for (const liveRequestId of liveRequestIds) {
        if (queueStartStopRequestedRef.current === startSequence) {
          queueStartStopConfirmedRef.current = startSequence;
          throw new Error('Queue start stopped by user.');
        }
      ...
      if (!hasCurrentBackendQueueSnapshot()) {
        throw createQueueAdmissionFailure('Backend queue state changed while preparing the queue. Try again after reconnect.', 'not-sent');
      }
      ...
      const stopDisposition = resolveQueueStartStopDisposition({
        startSequence,
        confirmedStopSequence: queueStartStopConfirmedRef.current,
        failedStopSequence: queueStartStopFailedRef.current,
        stopInFlight: queueStopInFlightRef.current,
      });
      if (stopDisposition.suppressStartError) {
        if (stopDisposition.restorePaused) setQueuePaused(true);
        for (const requestId of liveRequestIds) {
          queueAdmissionAttemptedRequestIdsRef.current.delete(requestId);
          queueAdmissionUncertainRequestIdsRef.current.delete(requestId);
          queueAdmissionObservedRequestIdsRef.current.delete(requestId);
        }
        logPowerPrompterDebug('queue:start:stoppedByControl', { requestIds: liveRequestIds }, { includeQueue: true });
        return;
      }
      ...
      activeQueueBatchRequestIdRef.current = '';
      activeQueueBatchGroupRequestIdsRef.current = [];
      if (!resolveQueueStartStopDisposition({
        startSequence,
        confirmedStopSequence: queueStartStopConfirmedRef.current,
        failedStopSequence: queueStartStopFailedRef.current,
        stopInFlight: queueStopInFlightRef.current,
      }).keepStopBusy) setQueueControlBusy(null);
    }
  };

  const executeHardStopComfyQueue = async (action: 'cancel' | 'clear') => {
    if (queueDestructiveActionBusy || queueStopInFlightRef.current) return;
    const stoppedStartSequence = resumeQueueInFlightRef.current ? queueStartSequenceRef.current : 0;
    const pendingBatchRequestId = activeQueueBatchRequestIdRef.current;
    if (stoppedStartSequence) queueStartStopRequestedRef.current = stoppedStartSequence;
    queueStopInFlightRef.current = true;
    ...
    try {
      ...
      let effectiveActiveRequestId = resolveActiveQueueControlRequestId(
        activeRequestId,
        backendQueueActivePromptRequestIdsRef.current,
        bridgeQueueStateRef.current.activeRequestIds,
      );
      const activeRequestIsLive = !!effectiveActiveRequestId;
      const hasOnlyPausedBackendWork = backendQueueSnapshotPausedRef.current
        && backendQueueSnapshotRequestIdsRef.current.size > 0
        && backendQueueActivePromptRequestIdsRef.current.size === 0
        && bridgeQueueStateRef.current.activeRequestIds.length === 0
        && bridgeQueueStateRef.current.pendingRequestIds.length === 0
        && bridgeQueueStateRef.current.pendingCount === 0;
      logQueueDebug('action:hardStop:resolved', {
        ...
        hasOnlyPausedBackendWork,
      });

      // Start can still be writing group history before it begins the upload.
      // The stop request above fences that start at its final pre-send check.
      if (stoppedStartSequence && !pendingBatchRequestId
        && backendQueueSnapshotRequestIdsRef.current.size === 0
        && !effectiveActiveRequestId
        && bridgeQueueStateRef.current.activeRequestIds.length === 0
        && bridgeQueueStateRef.current.pendingRequestIds.length === 0
        && bridgeQueueStateRef.current.pendingCount === 0
        && (action === 'cancel' || pendingBatchAdmissionsRef.current === 0)) {
        rejectAllPendingQueueRequests('Queue stopped by user.');
        clearQueueManagerLiveDisplay(`pre-upload ${action} completed`);
        setQueuePaused(false);
        setGenerationPreview((prev) => prev ? { ...prev, status: 'idle', updatedAt: Date.now() } : prev);
        scheduleGenerationPreviewHide();
        showToast(action === 'clear' ? 'Cleared queue.' : 'Canceled queue.', 'success');
        return;
      }
      ...
      if (hasOnlyPausedBackendWork
        ...
        if (!hasAcknowledgedPendingBatchControl({
          ...
          throw new Error('The backend did not confirm that the pending batch was stopped. Check the queue before retrying.');
        }
        if (pendingBatchRequestId && Array.isArray(result?.canceledBatchRequestIds)
          && result.canceledBatchRequestIds.includes(pendingBatchRequestId)) {
          queueStartStopConfirmedRef.current = stoppedStartSequence;
          rejectPendingQueueRequestById(pendingBatchRequestId, 'Queue stopped by user.');
        }
        const noBridgeSubmittedWork = bridgeQueueStateRef.current.activeRequestIds.length === 0
          && bridgeQueueStateRef.current.pendingRequestIds.length === 0
          && bridgeQueueStateRef.current.pendingCount === 0;
        if (noBridgeSubmittedWork && hasAcknowledgedUnsubmittedQueueControl({
          result, action,
          backendRequestIds: intendedBackendRequestIds,
          pendingBatchRequestId,
          pendingBatchGroupRequestIds,
        })) {
          rejectAllPendingQueueRequests('Queue stopped by user.');
        ...
        // Another client may have resumed this queue between our snapshot and
        // the control. Its current ComfyUI prompt still needs the normal path.
        const latestVisual = queueVisualStateRef.current;
        effectiveActiveRequestId = resolveActiveQueueControlRequestId(
          String(latestVisual?.requestId || '').trim(),
          backendQueueActivePromptRequestIdsRef.current,
          bridgeQueueStateRef.current.activeRequestIds,
        );
        activeIndex = Math.max(0, Math.floor(Number(latestVisual?.activeIndex) || 0));
      }
      ...
      if (stoppedStartSequence && !pendingBatchRequestId) {
        queueStartStopFailedRef.current = stoppedStartSequence;
        setQueuePaused(true);
      }
      ...
      queueStopInFlightRef.current = false;
      setQueueConfirmAction(null);
      setQueueControlBusy(null);
      setQueueingMode(null);
  ...
  const handleQueuePrompts = async (
    ...
    const queueSource = {
      ...
      fileLoadSequence: fileLoadRequestSeqRef.current,
    };
    const assertQueueSourceUnchanged = () => {
      if (hasQueueBuildSourceChanged(queueSource, {
        file: currentFileRef.current,
        documentSignature: getCardDocSignature(cardDocumentRef.current),
        fileLoadSequence: fileLoadRequestSeqRef.current,
      })) {
        throw new Error('The card changed while preparing this queue. Queue it again from the current card.');
      }
    };
    ...
    try {
      ...
      if (!hasCurrentBackendQueueSnapshot()) {
        throw new Error('Backend queue state changed while preparing prompts. Queue them again after reconnect.');
      }
      if (shouldBlockStageForPausedBackendQueue(queuePausedRef.current, backendQueueSnapshotRequestIdsRef.current)) {
