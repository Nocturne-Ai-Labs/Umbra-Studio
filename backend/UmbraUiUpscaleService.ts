...
export class UmbraUiUpscaleService {
  ...
  private async uploadInput(jobId: string, index: number, sourceName: string, bytes: Uint8Array): Promise<string> {
    ...
    if (!payload || typeof payload.name !== 'string' || !payload.name.trim()
      ...
      throw new Error('ComfyUI returned an invalid upscale upload response. Retry the operation.');
    }
    ...
    if (returnedName !== safeName || returnedSubfolder !== subfolder) {
      throw new Error('ComfyUI returned an invalid upscale upload path. Retry the operation.');
    }
